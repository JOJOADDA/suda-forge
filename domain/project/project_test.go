...
func TestProjectLifecycle(t *testing.T) {
	...
	if p.Slug != "hello-world" { t.Fatalf("slug = %q", p.Slug) }
	for _, next := range []Status{StatusPreparing, StatusReady, StatusRunning, StatusStopping, StatusStopped, StatusRunning} {
		if err := p.Transition(next, now); err != nil { t.Fatalf("transition to %s: %v", next, err) }
	}
}
...
func TestProjectRejectsIllegalTransition(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if err := p.Transition(StatusRunning, time.Now()); err == nil { t.Fatal("expected illegal transition error") }
}
