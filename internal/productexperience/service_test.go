...
func TestContextRecoveryAndImpact(t *testing.T) {
	...
	_, _ = s.Knowledge.UpsertNode(context.Background(), knowledge.Node{ID: "file.auth", ProjectID: "p1", Type: knowledge.File, Name: "auth.go", Path: "internal/auth.go"})
	_, _ = s.Knowledge.UpsertNode(context.Background(), knowledge.Node{ID: "api.login", ProjectID: "p1", Type: knowledge.API, Name: "login", Path: "/login"})
	_, _ = s.Knowledge.UpsertNode(context.Background(), knowledge.Node{ID: "table.users", ProjectID: "p1", Type: knowledge.Table, Name: "users"})
	_, _ = s.Knowledge.UpsertEdge(context.Background(), knowledge.Edge{ID: "file.api", ProjectID: "p1", From: "file.auth", To: "api.login", Type: knowledge.Implements})
	_, _ = s.Knowledge.UpsertEdge(context.Background(), knowledge.Edge{ID: "api.table", ProjectID: "p1", From: "api.login", To: "table.users", Type: knowledge.Writes})
